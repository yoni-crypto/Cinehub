...
export default function ClientPage({ year }: ClientPageProps) {
  ...
  useEffect(() => {
    ...
    loadMovies();
  }, [year, appliedGenre, appliedSortBy, page]);
  ...
  const hasActiveFilters = appliedGenre !== 'all' || appliedSortBy !== 'popularity.desc';
      ...
      <main className="max-w-[1600px] mx-auto px-3 sm:px-5 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold mb-4">
            Best Movies from {year}
          </h1>
          <p className="text-muted-foreground text-lg max-w-3xl">
            Discover the most popular movies and TV shows released in {year}. Stream all the best content from {year} in HD quality.
          </p>
        </div>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          >
            ...
            Filters
            {hasActiveFilters && <span className="ml-1 px-1.5 py-0.5 bg-red-600 text-white text-xs rounded-full">{[appliedGenre !== 'all', appliedSortBy !== 'popularity.desc'].filter(Boolean).length}</span>}
          </Button>
        </div>
