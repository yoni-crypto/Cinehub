  ...
  try {
    ...
    if (!tvShowId || !seasonNumber || !episodeNumber) {
      return NextResponse.json(
        { error: 'TV Show ID, Season Number, and Episode Number are required' },
        { status: 400 }
      );
    }

    const url = `https://moviesapi.club/tv/${tvShowId}-${seasonNumber}-${episodeNumber}`;
    console.log('Fetching TV show streaming URL:', url);
    ...
    if (!response.ok) {
      console.error('Failed to fetch TV show streaming data:', response.status, response.statusText);
      return NextResponse.json(
        { error: 'Failed to fetch TV show streaming data' },
    ...
    // Pattern 4: Standard vidora.stream embed URL (fallback)
    if (!embedUrl) {
    ...
    // Pattern 5: Any vidora.stream URL (fallback)
    if (!embedUrl) {
    ...
    // Pattern 6: Look for iframe with any streaming URL
    if (!embedUrl) {
      match = html.match(/<iframe[^>]*src=["']([^"']+)["'][^>]*>/i);
      if (match && (match[1].includes('stream') || match[1].includes('moviesapi.club'))) {
        embedUrl = match[1];
        console.log('Found with Pattern 6 (iframe streaming):', embedUrl);
      }
    }
    
    // Pattern 7: Look for any embed URL as last resort
    if (!embedUrl) {
      match = html.match(/src=["']([^"']*embed[^"']*)["']/i);
      if (match) {
        embedUrl = match[1];
        console.log('Found with Pattern 7 (any embed):', embedUrl);
      }
    }
    ...
    if (embedUrl) {
      ...
      return NextResponse.json(
        { error: 'High-quality streaming not available for this TV show episode. Please try another episode.' },
        { status: 404 }
      );
    }
  } catch (error) {
    console.error('Error fetching TV show streaming embed URL:', error);
    return NextResponse.json(
